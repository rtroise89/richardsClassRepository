...
*Outputs a matrix of values depending on the format of the input. Ex: If we simply add numbers seperated by comma, the matrix will output one row of x columns. Another is to assign sets of numbers to tags which forces those sets to columns.
```{r}
...
*Creates a vector of vectors, indexed by double brackets [[x]] followed by the nested vector [x]
```{r}
  a_list <- list(1,2,3)
  a_list[[2]]
  b_list <- list(c(1,2,3),2,5)
  b_list[[1]][3]
```

# matrix()

*With no other arguments, function will create a one dimensional matrix. Utilizing nrow and ncol creates more rows and columns.
```{r}
...
*Outputs the vector given in the function and assigns levels the indicies
```{r}
  alpha <- c(2,4,6,8,10)
  factor(alpha)
```
...
*Combines two vectors into distinct rows
```{r}
  apple <- c(1,2,3)
  orange <- c(2,4,6)
  rbind(apple, orange)
...
*Combines two vectors into distinct columns
```{r}
  apple <- c(1,2,3)
  orange <- c(2,4,6)
  cbind(apple, orange)
...
```{r}
 ...
 b <- c(2,3,4,6,8)
 ab <- data.frame(a,b)
# ggplot(ab, aes(x = a, y = b)) + geom_point()
```
